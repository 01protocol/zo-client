...
export class ZoDBPerpLiqUser extends ZoDBTradeUser {
  ...
  async _insertPerpLiqs(marketsDecoder: (string) => MarketInfo, marketKey) {
    while (this.perpLiqPagesParsed[marketKey]! <= this.perpLiqCurrentPage[marketKey]!) {
      ...
      for (const perpLiq of perpLiqs) {
        const assetsTransferred =
          -perpLiq.assetsToLiqor / Math.pow(10, marketsDecoder(perpLiq.baseSymbol).assetDecimals)
  ...
  async getPerpLiqHistory(page: number, marketsDecoder: (string) => MarketInfo, allMarkets: boolean, currentMarketKey: string) {
    const marketKey = allMarkets ? ALL_MARKETS : marketsDecoder(currentMarketKey).symbol
