...
export class ZoDBPnlUser extends ZoBaseUser {
  ...
  async _getPnls(page: number, marketKey: string) {
    ...
    const market = (marketKey == ALL_MARKETS || marketKey == ALL_HISTORY) ? null : marketKey
  ...
  async _insertPnls(marketKey: string, assetDecimalsDecoder: (sym) => number) {
    while (this.pnlPagesParsed[marketKey]! <= this.pnlCurrentPage[marketKey]!) {
      const pnls = await this._getPnls(this.pnlPagesParsed[marketKey]!, marketKey)
      ...
      for (const pnl of pnls) {
        ...
        const entryPrice = finalPrice - (pnl.isLong ? pnlNumber : -pnlNumber)
        const pnlPercentage =
          (((pnlNumber > 0 ? 1 : -1) * Math.abs(finalPrice - entryPrice)) /
            entryPrice) *
          100
  ...
  async getPnlHistory(
    page: number,
    currentMarketKey: string, assetDecimalsDecoder: (sym) => number,
    allMarkets: boolean,
    allHistory?: boolean,
  ) {
    const marketKey = allMarkets ? (allHistory ? ALL_HISTORY : ALL_MARKETS) : currentMarketKey
    ...
    if (allMarkets && this.pnlHistory[marketKey]!.length != 0) {
      return this.pnlHistory[marketKey]!
    ...
    return pnlHistory
      .filter((entry) => entry.size != 0)
      .slice(page * HISTORY_ENTRIES_PER_PAGE, (page + 1) * HISTORY_ENTRIES_PER_PAGE)
  }
}
