...
export class ZoDBTradeUser extends ZoDBPnlUser {
  ...
  async getTradesAndFunding(
    ...
    const positionsArr = this.loadPositionsArr(this.margin.state.markets, this.margin.state.indexToMarketKey)
    ...
    if (fundingOnly) {
      let fundings = await this._processTradesAndFunding(allTrades, positionsArr, fundingOnly, marketKey, page)
      let tradesNotFinishedFetching = !this.tradesParsed[marketKey]
      let enoughFundingsWereNotGenerated = fundings.length < HISTORY_ENTRIES_PER_PAGE
      while (tradesNotFinishedFetching && enoughFundingsWereNotGenerated) {
        ...
        fundings = await this._processTradesAndFunding(allTrades, positionsArr, fundingOnly, marketKey, page)
        tradesNotFinishedFetching = !this.tradesParsed[marketKey]
        enoughFundingsWereNotGenerated = fundings.length < HISTORY_ENTRIES_PER_PAGE
      }
      return fundings
    } else {
      if (this.tradesPagesParsed[marketKey]! <= page && !this.tradesParsed[marketKey]) {
        await this._insertTrades(marketKey)
      }
    }
    return await this._processTradesAndFunding(allTrades, positionsArr, fundingOnly, marketKey, page)
  }

  private async _processTradesAndFunding(allTrades: boolean, positions: PositionInfo[], fundingOnly: boolean, marketKey: string, page: number): Promise<Array<any>> {
    let tradeAndFundingHistory: Array<TradeHistoryEntry>
    if (allTrades) {
      tradeAndFundingHistory = []
      await Promise.all(positions.map(position => new Promise(async (res) => {
        tradeAndFundingHistory.push(
          ...((fundingOnly
            ? await this._combineTradesAndFunding(positions, position.marketKey, allTrades)
            : this.balanceHistory[ALL_MARKETS]!.filter(balance => balance.marketKey == position.marketKey))!),
        )
        res(true)
      })))
    } else {
    ...
    tradeAndFundingHistory = tradeAndFundingHistory.sort((a, b) => b.date.getTime() - a.date.getTime())
  ...
  private async _combineTradesAndFunding(
    ...
    let fundingHistory: any = this.fundingHistory[marketKey] ? this.fundingHistory[marketKey] : []
    let fundingIndex = 0
    let rollingSize = Math.abs(currentSize) > 1 / 1_000_000_000 ? currentSize : 0
    ...
    for (let i = 1; i < tradeHistoryEntries.length; i++) {
      while (
        fundingHistory[fundingIndex].date.getTime() > tradeHistoryEntries[i]!.date.getTime()
  ...
  async getFunding(marketKey: string) {
    ...
    if (this.fundingHistory[marketKey]) {
      const lastFundingTime = this.fundingHistory[marketKey]![this.fundingHistory[marketKey]!.length - 1].date.getTime()
