...
export class ZoUser extends ZoDBUser {
  ...
  positionPnLBasedOnMarkPrice(positionOrMarketKey: PositionInfo | string) {
    if (typeof positionOrMarketKey == "string") {
      return this.margin.positionPnLBasedOnMarkPrice(this.position(positionOrMarketKey))
    }
  ...
  async load(account: Wallet | Keypair, cluster: Cluster, connection: Connection, opts: {
    ...
    const stateKey = cluster == Cluster.Devnet ? ZO_DEVNET_STATE_KEY : ZO_MAINNET_STATE_KEY
    const state = await State.load(program, stateKey, opts.commitment)
    const margin = await Margin.load(program, state, null, wallet.publicKey, opts.commitment)
