...
export class ZoMarket {
	...
	static async load(
		...
		const decoded = (layoutOverride ?? this.getLayout(programId)).decode(
			data,
		)
		if (
	...
	async findBaseTokenAccountsForOwner(
		...
		if (
			...
			const [wrapped, unwrapped] = await Promise.all([
				this.findBaseTokenAccountsForOwner(
					connection,
					ownerAddress,
					false,
				),
			...
			if (unwrapped !== null) {
				return [
					{ pubkey: ownerAddress, account: unwrapped },
					...wrapped,
				]
			}
			return wrapped
	...
	async findQuoteTokenAccountsForOwner(
		...
		if (
			...
			const [wrapped, unwrapped] = await Promise.all([
				this.findQuoteTokenAccountsForOwner(
					connection,
					ownerAddress,
					false,
				),
	...
	async findOpenOrdersAccountsForOwner(
		...
		const openOrdersAccountsForOwner =
			await ZoOpenOrders.findForMarketAndOwner(
				connection,
				this.address,
				ownerAddress,
				this._programId,
	...
	async loadFills(connection: Connection, limit = 100) {
		...
		return events
			.filter(
				(event) =>
					event.eventFlags.fill && event.nativeQuantityPaid.gtn(0),
			)
...
export class ZoOpenOrders {
	...
	static fromAccountInfo(
		...
		if (
			!decoded.accountFlags.initialized ||
			!decoded.accountFlags.openOrders
		) {
