...
export default abstract class Margin extends MarginWeb3 {
	...
	get funding() {
		...
		for (const position of this.positions) {
			const positionFunding = this.positionFunding(position)
	...
	get balances(): { [key: string]: Num } {
		...
		balances["USDC"] = new Num(
			this._balances["USDC"]!.decimal.add(this.realizedPnL).add(
				this.funding,
			),
	...
	get isBankrupt() {
		if (this.isLiquidatable) {
			...
			for (const assetKey of Object.keys(this.state.assets)) {
				...
				const value =
					this.balances[assetKey]!.decimal.mul(price).toNumber()
				if (value > 1) {
					return false
				}
			}
	...
	private get collateralMaintenanceMarginInfo(): [Decimal, Decimal] {
		...
		for (const marketKey of Object.keys(this.balances)) {
			if (this.balances[marketKey]!.number < 0) {
				const { weight, weightedBorrow } =
					this.getWeightedBorrow(marketKey)
	...
	private get positionsTiedCollateral() {
		...
		for (const marketKey of Object.keys(this.state.markets)) {
			...
			tiedCollateral = tiedCollateral.add(
				this.state.markets[marketKey]!.baseImf.mul(
					openSize.add(posNotional),
				),
			)
		}
		return tiedCollateral
	...
	private get _ooInfos() {
		...
		for (const order of this.orders) {
			if (order.long) {
				posInfos[order.marketKey]!.long = posInfos[
					order.marketKey
				]!.long.add(order.coins.decimal)
			} else {
				posInfos[order.marketKey]!.short = posInfos[
					order.marketKey
				]!.short.add(order.coins.decimal)
			}
		}
	...
	static async loadAllMargins(
		...
		const marginAndControlSchemas =
			await this.loadAllMarginAndControlSchemas(program)
   ...
   */
    ...
    fundingAccrued: ${this.positionFunding(position).toNumber()}
...
Positions:[ ${
	"\n" +
			this.positions
				.filter(
					(position) =>
						position.coins.number != 0 ||
						position.realizedPnL.number != 0,
				)
