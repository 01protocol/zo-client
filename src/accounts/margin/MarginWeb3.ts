...
export default class MarginWeb3 extends BaseAccount<MarginClassSchema> {
   ...
   */
	...
	get currentSlots():SlotsState{
		return {
			stateSlot:this.state.slot,
			cacheSlot:this.state.cache.slot,
			marginSlot:this.slot,
			controlSlot:this.control.slot,
			specialOrdersSlot:this.specialOrdersAccount?this.specialOrdersAccount.slot:0
		}
	}
   ...
   */
	protected static async loadPrefetchedWeb3(
      ...
      (await this.getHeimdallPda(program.programId))
		const specialOrders =
	...
	static async loadFromAccountInfo(
		...
		const [heimdallKey, soKey] = await Promise.all([
			this.getHeimdallPda(program.programId),
			await SpecialOrders.getPda(st, data.authority, program.programId),
		])
		const [control, specialOrders] = await Promise.all([
   ...
   */
	...
	async loadSpecialOrders() {
		...
		const specialOrders = await Promise.all(
			this.specialOrdersAccount.data.entries.map(async (x) => {
				const perpMarket = keyToMarket[x.market.toString()]!
				const market = await this.state.getMarketBySymbol(
					perpMarket.symbol,
				)
		...
		const changeLog: Array<UserSpecialOrderChange> = []
		for (const oldOrder of this.specialOrders) {
			switch (getSpecialOrderStatus(oldOrder, specialOrders)) {
			case OrderChangeStatus.Changed:
				const orderChange: UserSpecialOrderChange = {
					curr: specialOrders.find((order) =>
		...
		for (const newOrder of specialOrders) {
			switch (getSpecialOrderStatus(newOrder, this.specialOrders)) {
			case OrderChangeStatus.Missing:
				changeLog.push({
					curr: newOrder,
					prev: null,
   ...
   */
	...
	private async subscribeToState(withBackup: boolean, stateLimit: number, cacheLimit: number) {
		const that = this
	...
	private async subscribeToControl(withBackup: boolean) {
		const that = this
	...
	async subscribeToSpecialOrdersAccount(withBackup: boolean) {

		if (this.specialOrdersAccount) {
			const that = this
			await this.specialOrdersAccount.subscribe(withBackup)
      //Note: when control modified only margin event is emitted
      this.specialOrdersAccount.eventEmitter!.addListener(
      	UpdateEvents.specialOrdersUpdated,
      	async (data) => {
      		const changeLog = [
      			...that.loadBalances(),
      			...that.loadPositions(),
	...
	async subscribe(withBackup = false, stateLimit = 1000, cacheLimit = 5000) {
		...
		await this.subscribeToSpecialOrdersAccount(withBackup)
		await this.subscribeToControl(withBackup)
		await this.subscribeToState(withBackup, stateLimit, cacheLimit)
		this.subLock.unlock()
	}
	...
	async unsubscribe() {
      ...
      if (this.specialOrdersAccount) {
      	await this.specialOrdersAccount.unsubscribe()
      }
   ...
   */
	async depositRaw(
		...
		return await this.rpc.deposit(repayOnly, amount, {
			accounts: {
   ...
   */
	async withdrawSol(vault: PublicKey, amount: BN, allowBorrow: boolean) {
		...
		return await this.rpc.withdraw(allowBorrow, amount, {
			accounts: {
				state: this.state.pubkey,
				stateSigner: this.state.signer,
				cache: this.state.cache.pubkey,
   ...
   */
    ...
    clientId?: BN
		...
		return await this.rpc.placePerpOrder(
			isLong,
			limitPrice,
			maxBaseQty,
			maxQuoteQty,
   ...
   */
    ...
    maxTs?: number
      ...
      market.decoded.perpType.toNumber() === 1
		...
		if (maxTs) {
			return await this.rpc.placePerpOrderWithMaxTs(
				isLong,
				limitPriceBn,
				maxBaseQtyBn,
				maxQuoteQtyBn,
   ...
   */
    ...
    clientId?: BN
		...
		return await this.rpc.cancelPerpOrder(
			orderId ?? null,
			isLong ?? null,
			clientId ?? null,
			{
   ...
   */
    ...
    serumMarket: PublicKey
      ...
      !market.quoteMintAddress.equals(stateQuoteMint)
		...
		return await this.rpc.swap(buy, allowBorrow, amount, minRate, {
			accounts: {
				authority: this.publicKey,
				state: this.state.pubkey,
				stateSigner: this.state.signer,
   ...
   */
	async settleFunds(symbol: string) {
		...
		return await this.rpc.settleFunds({
			accounts: {
				authority: this.publicKey,
				state: this.state.pubkey,
				stateSigner: this.state.signer,
	...
	private async createSpecialOrdersAccountIx(): Promise<[TransactionInstruction, PublicKey]> {
    ...
    limitPrice?: number
      ...
      limitPrice !== undefined
      	...
      	: isLong
      		? new BN(Math.round(triggerPrice * 1e6 * 20))
      		: new BN(Math.round((triggerPrice * 1e6) / 20))
	...
	async cancelSpecialOrder(id: number) {
		const o = this.specialOrders.find((o) => o.id == id)
		if (o === undefined) {
