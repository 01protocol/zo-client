...
export default abstract class BaseAccount<T> {
  ...
  protected async _subscribe(accountName: string) {
    ...
    this.backupSubscriberChannel = this.program.provider.connection.onAccountChange(this.pubkey, async (serializedAccount) => {
      const account = await this.program.account[accountName].coder.accounts.decode(
        accountName,
