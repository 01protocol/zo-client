...
export default abstract class BaseAccount<T> {
	...
	updateAccountOnChange = (processUpdate, that: BaseAccount<T>) => ((account, snapshotTime?) => {
		if (snapshotTime) {
		...
		if (difference < that.subscribeTimeLimit) {
			setTimeout(() => {
				that.updateAccountOnChange(processUpdate, that)(account, currTime)
	...
	protected async _subscribe(
		...
		if (!withBackup && (this.commitment == "confirmed" || this.commitment == "finalized")) {
