...
export default class Zamm extends BaseAccount<Schema> {
  ...
  private static getZammProgram(zoProgram: Program<Zo>): Program<ZammIdlType> {
    return new Program<ZammIdlType>(ZAMM_IDL, ZAMM_PROGRAM_ID, zoProgram.provider)
  }
  ...
  async subscribe(xSensitivity=100,ySensitivity=10): Promise<void> {
    ...
    const anchorEventEmitter = await this._subscribe("zamm", this.zammProgram)
    const that = this
    await this.zammMargin.eventEmitter?.addListener(UpdateEvents.marginModified, async () => {
      const oldX = this.X.mul(xSensitivity).round()
      const oldY = this.Y.mul(ySensitivity).round()
      const { X, Y, price } = await this.getXY()
      if (!X.mul(xSensitivity).round().eq(oldX) || !Y.mul(ySensitivity).round().eq(oldY))
        this.eventEmitter!.emit(UpdateEvents.zammModified, { X, Y, price })
    })
    await this.zammMargin.state.eqEmitter(this.marketSymbol)!.addListener(UpdateEvents.eventQueueModified, async () => {
      const oldX = this.X
      const oldY = this.Y
      const { X, Y, price } = await this.getXY()
  ...
  async unsubscribe() {
    try {
      ...
      await this.zammMargin.state.unsubscribeFromEventQueue(this.marketSymbol)
    } catch (_) {
      //
    }
  }
  ...
  async getXY() {
    const eventQueue = (await this.zammMargin.state.getZoMarketAccounts({
    ...
    const { eqY, eqX } = eventQueue.reduce((accum, event) => {
      if (event.control.toString() == control) {
        if (event.eventFlags.maker && event.eventFlags.fill) {
          if (event.eventFlags.bid) {
            return { eqY: accum.eqY.sub(event.nativeQuantityPaid), eqX: accum.eqX.add(event.nativeQuantityReleased) }
          } else {
            return { eqY: accum.eqY.add(event.nativeQuantityReleased), eqX: accum.eqX.sub(event.nativeQuantityPaid) }
          }
        }
      }
      return accum
    ...
    const signedCoins = position.coins.n.mul(new BN(position.isLong ? 1 : -1))
    const XN = zammX
      .add(eqX)
      .add(signedCoins)

    const signedPcoins = position.pCoins.n.mul(new BN(position.isLong ? -1 : 1))
    const YN = eqY
      .add(this.zammMargin.balances[USDC_SYMBOL]!.n)
      .add(signedPcoins)

    const X = (new Num(XN, this.zammMargin.state.markets[this.marketSymbol]!.assetDecimals)).decimal
  ...
  async marketArb(arberMargin: Margin, x: number) {
    const X = (new Num(x, this.zammMargin.state.markets[this.marketSymbol]!.assetDecimals)).n
    const y = x * this.zammMargin.state.markets[this.marketSymbol]!.indexPrice.number * 10
  ...
  async arbRaw(arberMargin: Margin, dx: BN, maxDy: BN) {
    const market = await this.zammMargin.state.getMarketBySymbol(this.marketSymbol)
    const zammOo = (await this.zammMargin.getOpenOrdersKeyBySymbol(this.marketSymbol))[0]

    const zammStateKey = (await PublicKey.findProgramAddress(
    ...
    return await this.zammProgram.rpc.arbZamm(dx, maxDy, {
      accounts: {
        ...
        authOo: (await arberMargin.getOpenOrdersKeyBySymbol(this.marketSymbol))[0],
