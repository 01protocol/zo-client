...
export async function getWrappedSolInstructionsAndKey(initialSmollAmount, provider): Promise<{
  createTokenAccountIx: TransactionInstruction, initTokenAccountIx: TransactionInstruction, closeTokenAccountIx: TransactionInstruction, intermediary: PublicKey,intermediaryKeypair: Keypair
}> {
  ...
  const rent = await provider.connection.getMinimumBalanceForRentExemption(TokenAccountLayout.span)
  ...
  return { createTokenAccountIx, initTokenAccountIx, closeTokenAccountIx, intermediary,intermediaryKeypair }
}
